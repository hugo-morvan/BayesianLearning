...
postDistribution_plot <- hist(phi, breaks=10, col="red", prob=TRUE, xlab="phi",
   main="Posterior distribution of the odds phi")

postDistribution_plot
...
sigsq_draws = 1 / rchisq(10000, df)

hist(sigsq_draws, prob=TRUE, xlab = expression(sigma^2), main=expression(paste("Posterior distribution for ", sigma^2)), breaks = 100)
...
hist(Gs, main = "Posterior distribution of G", breaks = 100 )
...
```{r 2.c}
print(quantile(Gs, probs = c(0.025, 0.975)))
...
minimum = 0 #adjust later
maximum = 10 #adjust later

delta = 0.01
kappaGrid <- seq(minimum, maximum, by = delta)


f <- function(kappa, mu, y){
  n = length(y)
  (exp(kappa * sum(cos(y - mu))) - kappa/2)/(besselI(kappa, nu = 0))^n
}
posterior = f(kappaGrid, mu, obs)
#normalizing:
...
print(post_mode)
plot(posterior ~ kappaGrid, pch = 20, xlab = expression(kappa), main = paste("Posterior distribution of K with mode = ", post_mode))
