...
hist(postPred,main="Posterior predictive distribution of Pr(y=0|x)",xlab="Pr(y=0|x)",ylab="Frequency",breaks=30)
...
SimulatePostPredBin <- function(Nsim,OptimRes,X,Nwomen){
  ...
  postPred <- rep(0,Nsim)
  for (i in 1:Nsim){
    postPred[i] <- rbinom(1, Nwomen, 1/(1+exp(-X%*%postDraws[i,]))) # Pr(y=0|x) 
  }
  return(postPred)
}
...
hist(postPred,main="Posterior predictive distribution of number of women not working",xlab="Number of women",ylab="Frequency")
